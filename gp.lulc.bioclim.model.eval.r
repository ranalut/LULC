...
for (i in 1:21) # 1:length(spp$BBL_ABBREV) # c(1,2,5:21)
{
	...
	pa.bird.data <- data.frame(temp,pa.bird.data[,11:27])
		
	if (include.backcast=='n')
	{
		load(paste(workspace.d,'/Models/gp.lulc.bioclim.',type,'.',ver,'.92.',species,'.rdata',sep=''))
		cat('\nloaded... ',species,' ',the.radius,'\n')
	
		temp <- as.integer(rownames(pa.bird.data))
		if (type=='brt') { evaluation <- model.eval.binary(the.model=brt.model, covariates=pa.bird.data[temp%in%train.rows & pa.bird.data$count_yr >= 92,c(7,10:46)], test.cov=pa.bird.data[temp%in%test.rows & pa.bird.data$count_yr >= 92,c(7,10:46)], n.trees=brt.model$n.trees) }
		save(evaluation,file=paste(workspace.d,'/Models/gp.lulc.bioclim.',type,'.eval.',ver,'.92.',species,'.rdata',sep=''))
	}
	else
	{
		load(paste(workspace.d,'/Models/gp.lulc.bioclim.',type,'.',ver,'.',species,'.rdata',sep=''))
		cat('\nloaded... ',species,' ',the.radius,'\n')
		...
		if (type=='brt') { evaluation <- model.eval.binary(the.model=brt.model, covariates=pa.bird.data[train.rows,c(7,10:46)], test.cov=pa.bird.data[test.rows,c(7,10:46)], n.trees=brt.model$n.trees) }
		save(evaluation,file=paste(workspace.d,'/Models/gp.lulc.bioclim.',type,'.eval.',ver,'.',species,'.rdata',sep=''))
	}
...
if (include.backcast=='n') 
{ 
	write.csv(output,paste(workspace.d,'/Models/EvaluationTables/gp.lulc.bioclim.',type,'.eval.',ver,'.92.csv',sep=''))
}
if (include.backcast=='y')
{
	write.csv(output,paste(workspace.d,'/Models/EvaluationTables/gp.lulc.bioclim.',type,'.eval.',ver,'.csv',sep=''))
}
# stop('cbw')

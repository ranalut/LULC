...
cbc <- readOGR(dsn=workspace.z,layer='CBC_circles_alb',encoding='ESRI Shapefile')
cat('cbc pts loaded\n')
the.crop <- raster(paste(workspace.z,'/gp_backcast_1938_1992/gp_lcyear_1992.tif',sep=''))
test <- extract(the.crop,cbc)
cbc <- cbc[is.na(test)==FALSE & test!=0,]
cbc@data$abbrev <- as.character(cbc@data$abbrev)
pt.yr <- read.csv(paste(workspace.z,'/CirclesByYear.csv',sep=''), stringsAsFactors=FALSE)
...
# Load LULC data
load(paste(workspace.d,'/Maurer2010Historical/maurer.hist.cbc.bioclim.rdata',sep='')) # Called hist.bioclim
...
spp <- read.csv(paste(workspace.z,'/gp_focal_spp_list_v',ver,'.csv',sep=''), stringsAsFactors=FALSE, row.names=1)
...
for (i in 1:dim(spp)[1])
{
	...
	save(pa.bird.data, file=paste(workspace.d,'/Maurer2010Historical/Species/gp.bioclim.',ver,'.',species,'.rdata',sep=''))
	# stop('cbw')
	cat(species,' ')
}
cat('\n')
